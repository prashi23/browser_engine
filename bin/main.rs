...
fn get_html() -> Vec<dom::Node> {
    ...
    let nodes = html_parse::HtmlParser::new(&html_input).parse_nodes(); //to traverse the string and pass all the information
    nodes//return nodes
}
...
fn get_css() -> css::Stylesheet {
    ...
    let mut css_input = String::new(); // new mutable string
    file_reader.read_to_string(&mut css_input).unwrap();

    let stylesheet = css_parser::CssParser::new(&css_input).parse_stylesheet(); //to call css parser
    stylesheet
}
